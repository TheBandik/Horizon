...
export function MediaEditModal({
    ...
    const headerSub = [
        item?.originalTitle && item.originalTitle !== headerTitle
            ? item.originalTitle
            : null,
    ...
    const itemScope = useMemo(
        () => resolveScopeByMediaTypeName(item?.mediaType?.name),
        [item]
    );

    const filteredStatuses = useMemo(() => {
        if (!itemScope) {
            return statuses.filter((s) => s.scope === "ALL");
        }

        return statuses.filter(
            (s) => s.scope === "ALL" || s.scope === itemScope
        );
    }, [statuses, itemScope]);
    ...
    useEffect(() => {
        if (!opened) return;
        onStatusIdChange(null);
    }, [opened, item?.id, onStatusIdChange]);
                ...
                <Stack gap={8}>
                    ...
                    <Combobox
                        ...
                        onOptionSubmit={(value) => {
                            const picked = filteredStatuses.find(
                                (s) => String(s.id) === value
                            );
                            if (!picked) return;
                    ...
                    >
                        <Combobox.Target>
                            <TextInput
                                ...
                                readOnly
                                rightSection={
                                    statusesLoading ? <Loader size="xs"/> : null
                                }
                                onFocus={() =>
                                    statusCombobox.openDropdown()
                                }
                                onClick={() =>
                                    statusCombobox.openDropdown()
                                }
